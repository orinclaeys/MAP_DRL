...
def writeToDb(node, topic, value, tmstp):
    ...
    with open(f"{node}{topic.upper()}.txt", "a") as f:
        ...
        requests.put(f"http://localhost:3000/api/v1/resources/nodes/{topic}", json=data)


def writeDiagnostics(data):
    diagnostics = requests.put(
        "http://localhost:3000/api/v1/resources/decisions", json=data)
